...
class ThetaPostFitPlot():
    ...
    def prepare_pull_graph(n_items, post_fit_items):
       ...
       # for i, (_, (val, err)) in enumerate(post_fit_items):
        for i, (name, (val, err)) in enumerate(post_fit_items):
            if 'TTbar' in name:
                if err > 1. or err < -1.:
                    print bcolors.WARNING + 'WARNING: TTbar post fit uncertaity larger than the input uncertainty', err,'(',name,')'+ bcolors.ENDC
            x, y = val, i + 1.5
            g.SetPoint(i, x, y)
    ...
    def prepare_band_graphs(n_items):
        ...
        g68.SetFillColor(ROOT.kGreen+1)
