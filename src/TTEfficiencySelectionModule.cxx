...
private:
  ...
  //histograms
  //std::unique_ptr<BTagMCEfficiencyHists<CSVBTag>> hists_btag_eff, hists_btag_medium_eff, hists_subjet_btag_eff;
...
TTEfficiencySelectionModule::TTEfficiencySelectionModule(Context & ctx){
  ...
  //Electron ID needs some update for CMSSW10
  // ElectronId eleid    = AndId<Electron>(ElectronID_Spring16_medium_noIso, PtEtaSCCut(55., 2.4);

  //ElectronId eleid = AndId<Electron>(ElectronID_MVAGeneralPurpose_Spring16_tight, PtEtaCut(50., 2.1));

  JetId jetid = AndId<Jet>(JetPFID(JetPFID::WP_LOOSE_CHS), PtEtaCut(30.0, 2.4));
...
//  jet_lepton_cleaner.reset(new JetLeptonCleaner(ctx, JEC_AK4));
  ...
  */
  ...
  //histograms
   //hists_btag_eff.reset(new BTagMCEfficiencyHists<CSVBTag>(ctx,"BTagLoose",CSVBTag::WP_LOOSE));
   //hists_btag_medium_eff.reset(new BTagMCEfficiencyHists<CSVBTag>(ctx,"BTagMedium",CSVBTag::WP_MEDIUM));
   //hists_subjet_btag_eff.reset(new BTagMCEfficiencyHists<CSVBTag>(ctx,"SubjetBTag",CSVBTag::WP_LOOSE, "topjets") );
